...
function navPainter (items = []) {
    const articleNav = document.getElementById('article-nav-drop-down');
    ...
    items.forEach((item) => {
        ...
        articleNav.appendChild(divider);
    });
};
...
function loadArticleNav (noteDiv) {
    ...
    if (headers.length > 0) {
        ...
        document.getElementById('article-sections-nav').classList.remove('sticky-top')
    };
};